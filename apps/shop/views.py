...
from apps.shop.serializers import CheckoutSerializer, OrderSerializer, CategorySerializer, ProductSerializer, \
    OrderItemSerializer, ToggleCartItemSerializer, ReviewSerializer
...
# описывает эндпоинт для оформления заказа.
# Он валидирует данные, получает товары из корзины, создает заказ и связывает его с товарами,
# а затем возвращает данные о созданном заказе
...
class ReviewsView(APIView):
    ...
    @extend_schema(
        summary="Review",
        description="""
            This endpoint allow users to check all reviews for a product.
        """,
        tags=tags,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if Review.objects.filter(user=user, product=product).exists():
            return Response({"message": "Review is alreade exists"}, status=404)
        new_review = Review.objects.create(**serializer.validated_data)
    ...
    def patch(self, request, *args, **kwargs):
        ...
        serializer = self.serializer_class(instance=review, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
    ...
    def delete(self, request, *args, **kwargs):
        try:
            review = Review.objects.get(user=request.user, product=request.product)
        except Review.DoesNotExist:
            return Response({"message": "Review not found"}, status=404)

        review.delete() # "мягкое удаление" / отключение отзыва
