...
class SellerProductsView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        seller = Seller.objects.get_or_none(user=request.user)  # добавить is_approved=True для продакшена
        if not seller:
...
class SellerProductView(APIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if not product:
            return Response(data={"message": "Product does not exist"}, status=404)
        elif request.data["seller"] != product.seller:
            return Response(data={"message": "Access is denied"}, status=403)
        product.delete()
        return Response(data={"message": "Product deleted successfully"}, status=200)


# представление возвращает список всех заказов, связанных с конкретным продавцом
...
# представление возвращает список элементов заказа (товаров для конкретного заказа), принадлежащего данному продавцу
